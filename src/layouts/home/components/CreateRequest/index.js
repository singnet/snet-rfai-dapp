...
class CreateRequest extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.props.ServiceRequest !== prevProps.ServiceRequest ||
      prevState.dataKeyTokenBalance !== this.state.dataKeyTokenBalance
    ) {
      this.setBlockNumber();
      this.setTokenBalance(this.props.ServiceRequest);
    }
  ...
  handleCreateButton(event, needConfirmation) {
    ...
    const expiration =
      parseInt(this.state.blockNumber, 10) +
      this.helperFunctions.computeBlocksFromDates(new Date(), this.state.expirationDate);
    ...
    if (
      ...
      this.state.isValidGitHanlde === true &&
      ...
      fetch(ipfsURL, body)
        ...
        .then(data => {
          ...
          const stackId = this.contracts.ServiceRequest.methods["createRequest"].cacheSend(
  ...
  createToast() {
    ...
    toast.info(<TransactionResult toastId={tId} key={this.state.stackId} stackId={this.state.stackId} />, {
      toastId: tId,
      autoClose: false,
    });
  }
  ...
  renderRightPane() {
    if (this.state.selectedLeftNav === "nav1") {
      return (
        <div className="singularity-content">
          <p>
            ...
            Requests should be detailed enough to allow discussion and development. Requests accepted by the foundation
            will be raised as a github pull request to the <a href="https://github.com/singnet/rfai-proposal">RFAI</a>{" "}
            repository.
          </p>
          <p>
            We would like to have an objective and measurable acceptance criteria (get accuracy above X% of this data,
            etc). The foundation will review and approve requests which will appear on here
          </p>
        </div>
      );
      ...
      return (
        <div className="singularity-content">
          <div>
            <li>Provide a title and description along with the acceptance criteria</li>
            <li>
              In order to incentivize people to develop solutions we require that you back your requests with AGI
              tokens. The tokens will be distributed to the accepted solutions. See the Submission Evaluation process
              for more details
            </li>
            <li>
              Provide an expiry date for the request. Meaning the date post which you can withdraw your funds if no
      ...
      <div className="create-req-error-box">
        {typeof this.state.tokenBalance != "undefined" && parseInt(this.state.tokenBalance) === 0 ? (
          <div className="no-balance-text">
            ...
            <div className="add-more-funds-btn">
              <button onClick={event => this.handleRedirect(event, "myaccount")} className="blue">
                add funds
              </button>
            </div>
          </div>
        ...
        <form className="pure-form pure-form-stacked create-request-form">
          ...
          <div className="row create-new-req">
            ...
            <div className="col-md-9">{this.renderRightPane()}</div>
          </div>
        </form>
        {/* </Paper> */}
